...
class Parser {
    ...
    */
    ...
    public void generateWrapper(File targetDir, String packageName) {
        ...
        List<TableMapping> subclasses = mainMap.getSubClasses();
        for(TableMapping tm : subclasses)
            try {
    ...
    private void wrapFromTableMapping(TableMapping map, File targetDir, String packageName) {
        ...
        try {
            File target = new File(targetDir, className + ".java");
            if(target.exists()) {
                HBMigrate.log(target.getAbsolutePath() + " already exists, skipping");
                return;
            }
            HBMigrate.log("Generating " + target.getAbsolutePath());
            PrintWriter out = new PrintWriter(new FileOutputStream(target));
            
            String packTLD = packageName.split("\\.")[0];
            String targetPack = targetDir.getAbsolutePath().substring(
                    targetDir.getAbsolutePath().indexOf(packTLD)).replace(File.separator, ".");
            out.println();
            out.println("/* Generated by HBMigrate */");
            out.println();
            out.println("package " + targetPack + ";");
            out.println();
            out.println("import " + packageName + "." + map.getClassName() + ";");
            ...
            for(Mapping m : map.getMappings()) {
                out.println(m.getAnnotations());
                out.println(m.getJavaType() + " " + m.getJavaName() + ";\n");
            }
            out.println("}");
